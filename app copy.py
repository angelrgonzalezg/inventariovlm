...
DB_NAME = resource_path("inventariovlm.db")
...
def init_db():
    ...
    # Si la tabla ya existía sin la columna 'location', añadirla
    cur.execute("PRAGMA table_info(inventory_count)")
    cols = [r[1] for r in cur.fetchall()]
    if "location" not in cols:
        try:
            cur.execute("ALTER TABLE inventory_count ADD COLUMN location TEXT")
        except Exception:
            pass
...
def guardar():
    try:
        ...
        # Fecha seleccionada y ubicación (si DateEntry no disponible, usar texto)
        if DateEntry:
        ...
        location = (entry_location.get().strip()[:25]) if 'entry_location' in globals() else ''
    except ValueError:
    ...
    cur.execute("""
        ...
        (counter_name, code_item, magazijn, winkel, total, current_inventory, difference, location, count_date)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (name, code, magazijn, winkel, total, actual, diff, location, selected_date.isoformat()))
...
def mostrar_registros():
    ...
    def on_seleccionar(event=None):
        ...
        if not vals:
            return
        # vals: id, counter_name, code_item, description, magazijn, winkel, total, current_inventory, difference, location, count_date
        ...
        edit_location.delete(0, tk.END); edit_location.insert(0, vals[9])
        edit_date.delete(0, tk.END); edit_date.insert(0, vals[10])
    ...
    def actualizar_registro():
        ...
        cur.execute("""
            ...
            SET counter_name=?, code_item=?, magazijn=?, winkel=?, total=?, current_inventory=?, difference=?, location=?, count_date=?
            WHERE id=?
        """, (counter, code, magazijn, winkel, total, current_inv, diff, edit_location.get().strip()[:25], date_txt or datetime.now().isoformat(), id_reg))
        conn.commit()
        conn.close()
        cargar_datos()
    ...
    edit_location = ttk.Entry(frm, width=25)
    ...
    ttk.Label(frm, text="Location").grid(row=0, column=8, padx=3)
    edit_location.grid(row=1, column=8, padx=3)
    ttk.Label(frm, text="Date").grid(row=0, column=9, padx=3)
    edit_date.grid(row=1, column=9, padx=3)
...
style = ttk.Style()
style.theme_use("clam")  # clam SÍ permite colores
...
# Date (selector) and Location
ttk.Label(frame, text="Date:").grid(row=3, column=0, sticky="w", padx=(0,6), pady=2)
if DateEntry:
    date_entry = DateEntry(frame, width=12)
else:
    date_entry = ttk.Entry(frame)
    date_entry.insert(0, datetime.now().date().isoformat())
date_entry.grid(row=3, column=1, sticky="w", pady=2)

ttk.Label(frame, text="Location:").grid(row=4, column=0, sticky="w", padx=(0,6), pady=2)
entry_location = ttk.Entry(frame)
entry_location.grid(row=4, column=1, sticky="ew", pady=2)

# Current inventory
lbl_current = ttk.Label(frame, text="Current inventory: ")
lbl_current.grid(row=5, column=1, sticky="w", pady=2)

# Magazijn
label_mag = ttk.Label(frame, text="Magazijn:")
label_mag.grid(row=6, column=0, sticky="w", padx=(0,6), pady=2)
entry_mag = ttk.Entry(frame)
entry_mag.grid(row=6, column=1, sticky="ew", pady=2)
entry_mag.insert(0, "0")

# Winkel
label_win = ttk.Label(frame, text="Winkel:")
label_win.grid(row=7, column=0, sticky="w", padx=(0,6), pady=2)
entry_win = ttk.Entry(frame)
entry_win.grid(row=7, column=1, sticky="ew", pady=2)
entry_win.insert(0, "0")
...
###btn_row.grid(row=6, column=0, columnspan=2, sticky="ew", pady=(6, 4))
btn_row.grid(
    row=8,
    column=0,
    columnspan=2,
    pady=(6, 4),
    sticky="n"   # 👈 centrado, no estirado
)
...
for i in range(4):
    btn_row.columnconfigure(i, weight=0)
...
btn_import.grid(row=0, column=0, padx=8)
...
btn_export.grid(row=0, column=1, padx=8)
...
btn_registros.grid(row=0, column=2, padx=8)
